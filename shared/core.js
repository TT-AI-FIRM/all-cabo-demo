...
(function () {
  ...
  const CIUDADES = [['Houston, TX', 'US'], ['Los Angeles, CA', 'US'], ['Dallas, TX', 'US'], ['Phoenix, AZ', 'US'], ['Denver, CO', 'US'], ['Seattle, WA', 'US'], ['San Diego, CA', 'US'], ['Chicago, IL', 'US'], ['Calgary, AB', 'CA'], ['Vancouver, BC', 'CA'], ['Toronto, ON', 'CA'], ['Ciudad de México', 'MX'], ['Austin, TX', 'US'], ['New York, NY', 'US'], ['San Francisco, CA', 'US']];
  const CHOFERES = [
  ...
  const AEROLINEAS = [['UA', 'United', 'Houston (IAH)'], ['AA', 'American', 'Dallas (DFW)'], ['AS', 'Alaska', 'Seattle (SEA)'], ['DL', 'Delta', 'Atlanta (ATL)'], ['WN', 'Southwest', 'Denver (DEN)'], ['WS', 'WestJet', 'Calgary (YYC)'], ['AC', 'Air Canada', 'Vancouver (YVR)'], ['AM', 'Aeroméxico', 'Ciudad de México (MEX)'], ['Y4', 'Volaris', 'Guadalajara (GDL)'], ['UA', 'United', 'Los Angeles (LAX)'], ['AA', 'American', 'Phoenix (PHX)'], ['DL', 'Delta', 'Salt Lake City (SLC)']];
  ...
  function semilla() {
    ...
    [...hoyV.map(x => ({ ...x, fecha: H })), ...proxV].forEach((x, i) => {
      ...
      const vuelo = x.vuelo ? (db.vuelos.find(f => f.num === x.vuelo) || { num: x.vuelo, aerolinea: 'United', origen: 'Houston (IAH)', programada: hmDe(minutosDe(x.hora) - 35), estado: 'a tiempo', minutos: 0 }) : null;
