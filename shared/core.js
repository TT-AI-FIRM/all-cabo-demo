...
(function () {
  ...
  const MAPAS = {
    minimal: { nombre: { es: 'Minimal', en: 'Minimal' }, capas: [{ url: ESRI + 'Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}', maxNativeZoom: 16, maxZoom: 19 }, { url: ESRI + 'Canvas/World_Light_Gray_Reference/MapServer/tile/{z}/{y}/{x}', maxNativeZoom: 16, maxZoom: 19, opacity: .9 }] },
  ...
  function estiloMapa(m, estilo) { if (!window.L) return; (m._acsCapas || []).forEach(l => m.removeLayer(l)); const def = MAPAS[estilo] || MAPAS.minimal; m._acsCapas = def.capas.map((c, i) => L.tileLayer(c.url, { maxZoom: c.maxZoom || 19, maxNativeZoom: c.maxNativeZoom || c.maxZoom || 19, opacity: c.opacity == null ? 1 : c.opacity, subdomains: c.subdomains || 'abc', attribution: i === 0 ? (def.attr || ATTR) : '' }).addTo(m)); m._acsEstilo = MAPAS[estilo] ? estilo : 'minimal'; if (!m._acsFijo) { try { localStorage.setItem('acs_mapa2', m._acsEstilo); } catch (e) { } } if (m._acsBtn) m._acsBtn.textContent = def.nombre[m._acsIdioma || 'es']; }
  function mapa(el, opts) { opts = opts || {}; const m = L.map(el, Object.assign({ zoomControl: false, attributionControl: true }, opts.leaflet || {})); m._acsIdioma = opts.idioma || 'es'; m._acsFijo = !!opts.fijo; let estilo = opts.estilo || 'minimal'; if (!opts.fijo) { try { estilo = localStorage.getItem('acs_mapa2') || estilo; } catch (e) { } } estiloMapa(m, estilo);
    if (opts.selector !== false) { const wrap = el.parentElement; const b = document.createElement('button'); b.className = 'mapstyle press'; b.type = 'button'; b.title = m._acsIdioma === 'en' ? 'Map style' : 'Estilo de mapa'; b.textContent = (MAPAS[m._acsEstilo] || MAPAS.minimal).nombre[m._acsIdioma]; b.onclick = () => { const ks = Object.keys(MAPAS); estiloMapa(m, ks[(ks.indexOf(m._acsEstilo) + 1) % ks.length]); }; m._acsBtn = b; (wrap || el).appendChild(b); }
    return m; }
